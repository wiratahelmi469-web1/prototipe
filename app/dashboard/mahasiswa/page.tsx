    ...
    <Workspace id="mahasiswa_dashboard_viewport">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white border border-stone-200 rounded-2xl p-6 shadow-xs relative overflow-hidden" id="student_welcome_panel">
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-full bg-navy-tint text-navy flex items-center justify-center font-extrabold text-lg uppercase shadow-inner">
                {user.name[0]}
              </div>
              <div>
            ...
            <div className="grid grid-cols-3 gap-4 border-t border-stone-105 mt-6 pt-5">
              <div className="text-center md:text-left">
                ...
                <span className="text-xl font-black text-navy font-mono block mt-1">{joinedEvents.length}</span>
              </div>
              <div className="text-center md:text-left border-l border-r border-stone-105 px-2">
                ...
                <span className="text-xl font-black text-[#F5A623] font-mono block mt-1">{statsSertifikatSiap}</span>
              </div>
              <div className="text-center md:text-left">
          ...
          <div className="bg-white border border-stone-200 rounded-2xl p-5 shadow-xs">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xs font-black uppercase text-stone-450 tracking-wider">EVENT SAYA DIIKUTI</h3>
              <Link href="/dashboard/mahasiswa/riwayat" className="text-xs font-bold text-navy hover:text-navy-mid flex items-center gap-1 transition-all">
            ...
            {joinedEvents.length === 0 ? (
              <div className="py-8 text-center text-stone-400 border border-dashed border-stone-200 rounded-xl bg-stone-50/50">
                ...
                <Link href="/events" className="mt-3 inline-flex px-3.5 py-1.5 bg-navy hover:bg-navy-mid text-white rounded-lg text-[11px] font-bold shadow-xs">
                ...
                {joinedEvents.map((evt) => (
                  <div key={evt.id} className="p-3.5 border border-stone-150/80 hover:border-stone-250 rounded-xl flex items-center justify-between gap-4 transition-all" id={`my_event_${evt.id}`}>
                    <div className="space-y-0.5">
                      <span className="text-[9px] font-bold text-navy uppercase tracking-widest">{evt.category}</span>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-navy text-white border border-navy-mid rounded-2xl p-5 shadow-lg flex flex-col justify-between min-h-[340px]" id="gemini_advisor_panel">
            <div>
              <span className="inline-flex items-center gap-1 text-[8px] uppercase tracking-widest font-black text-[#F5A623] bg-navy-mid/60 px-2.5 py-0.5 rounded-sm border border-navy-light/10 mb-3.5">
                <Sparkles className="w-3.5 h-3.5 text-gold fill-gold shrink-0" />
                Asisten AI Gemini
              </span>
              ...
              {/* Bot Response Bubble */}
              <div className="bg-navy-mid/80 border border-navy-light/10 p-3 rounded-xl text-[11px] mt-4 leading-relaxed font-semibold italic text-stone-100 max-h-[140px] overflow-y-auto custom-scrollbar">
                {aiResponse}
              </div>
            </div>
            ...
            <form onSubmit={askGemini} className="relative mt-4">
              <input
                ...
                className="w-full text-xs bg-navy-mid/40 border border-navy-light/25 rounded-xl py-2.5 pl-4 pr-11 outline-hidden focus:border-[#F5A623] font-semibold text-white placeholder-stone-400"
                value={aiPrompt}
              ...
              <button
                type="submit"
                disabled={aiLoading}
                className="absolute right-1 top-1 p-2 bg-[#F5A623] hover:bg-[#F5A623]/80 rounded-lg text-navy transition-colors font-bold"
              >
              ...
              {notifications.map((notif) => (
                <div key={notif.id} className="py-2.5 first:pt-0 last:pb-0 space-y-1">
                  <div className="flex justify-between items-center text-[9px] font-mono">
                    <span className="font-extrabold text-navy uppercase tracking-widest">{notif.category}</span>
