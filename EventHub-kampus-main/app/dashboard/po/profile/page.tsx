// CREATED: app/dashboard/po/profile/page.tsx
"use client";
...
export default function PoProfilePage() {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    addToast("Profil Project Officer berhasil diperbarui!", "success");
  };
  ...
  return (
    <div className="space-y-6 select-none animate-fadeIn max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          <ArrowLeft className="w-4 h-4" /> Kembali ke Dasbor PO
        </button>
      </div>
      ...
      <div className="bg-[#114E8D] rounded-3xl p-6 text-white border-b-4 border-amber-400 shadow-md">
        <span className="bg-amber-400 text-slate-900 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded bg-opacity-95 shadow inline-flex items-center gap-1 mb-2.5">
          <Shield className="w-3.5 h-3.5" /> Otoritas Hukum Project Officer
        </span>
        <h1 className="text-xl sm:text-2xl font-black uppercase tracking-tight">Profil Project Officer</h1>
        <p className="text-[11px] sm:text-xs text-slate-205 mt-1 max-w-xl font-medium leading-relaxed">
          Atur metadata profil penanggung jawab pembubuh persetujuan sertifikat dan proposal kegiatan kemahasiswaan.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm flex flex-col items-center justify-center text-center space-y-4">
          <div className="w-24 h-24 rounded-full bg-rose-600 text-white flex items-center justify-center font-black text-3xl border-4 border-white shadow-md select-none">
            {getInitials(nama)}
          </div>

          <div>
            <h2 className="text-sm font-black text-slate-800 uppercase leading-snug">{nama}</h2>
            <p className="text-[11px] text-slate-450 font-mono font-bold mt-0.5">{nim || "NIP Tidak Terbaca"}</p>
          </div>
          ...
          <div className="flex flex-col gap-2 w-full pt-4 border-t border-dashed">
            <div className="flex justify-between text-[11px] font-semibold text-slate-600">
              ...
              <span className="bg-rose-100 text-rose-800 px-2.5 py-0.5 rounded text-[10px] font-black uppercase tracking-wider border border-rose-200">
                Project Officer
              </span>
            </div>
            <div className="flex justify-between text-[11px] font-semibold text-slate-600">
              <span className="text-slate-400 uppercase font-mono">Gabung:</span>
              <span className="text-slate-700 font-mono">{user.tanggalBergabung || "10 Januari 2026"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-2 bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm">
          <h3 className="text-xs font-mono font-black uppercase text-[#114E8D] border-b pb-2 mb-4 tracking-wider">
             FORMULIR KOORDINAT DIKTAT PROJECT OFFICER
          </h3>
          ...
          <form onSubmit={handleSave} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className="text-[9.5px] font-black uppercase text-slate-400 font-mono tracking-widest block">
                  NIP / Kode Otoritas <span className="text-rose-500">*</span>
                </label>
                <div className="relative">
                  <span className="absolute inset-y-0 left-0 flex items-center pl-3.5 pointer-events-none text-slate-400">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Contoh: Pembina Bidang Kemahasiswaan..."
                    className="w-full text-slate-700 font-semibold pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:border-[#114E8D] outline-none transition-all text-xs h-11"
                  />
                </div>
              </div>
