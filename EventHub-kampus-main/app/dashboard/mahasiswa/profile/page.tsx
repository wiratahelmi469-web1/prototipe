// CREATED: app/dashboard/mahasiswa/profile/page.tsx
"use client";

import React, { useState, useEffect } from "react";
import { useRouter } from "next/navigation";
import { useAuth } from "../../../../context/AuthContext";
import { User, Phone, BookOpen, Mail, Shield, Save, ArrowLeft, Calendar } from "lucide-react";
...
export default function MahasiswaProfilePage() {
  ...
  const handleSave = (e: React.FormEvent) => {
    ...
    // Sync context profile info (AuthContext updates user.nama & user.nim globally)
    updateUserProfile(nama.trim(), nim.trim());
    addToast("Profil mahasiswa berhasil diperbarui di server lokal!", "success");
  };
    ...
    <div className="space-y-6 select-none animate-fadeIn max-w-4xl mx-auto">
      ...
      <div className="bg-[#114E8D] rounded-3xl p-6 text-white border-b-4 border-amber-400 shadow-md">
        <span className="bg-amber-400 text-slate-900 text-[10px] font-black uppercase tracking-widest px-3 py-1 rounded bg-opacity-95 shadow inline-flex items-center gap-1 mb-2.5">
          <Shield className="w-3.5 h-3.5" /> Pengaturan Identitas Mahasiswa
        </span>
        <h1 className="text-xl sm:text-2xl font-black uppercase tracking-tight">Profil Saya</h1>
        <p className="text-[11px] sm:text-xs text-slate-200 mt-1 max-w-xl font-medium leading-relaxed">
          Kelola informasi kontak dan program studi Anda untuk integrasi data sertifikat digital secara otomatis.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm flex flex-col items-center justify-center text-center space-y-4">
          <div className="w-24 h-24 rounded-full bg-amber-400 text-slate-950 flex items-center justify-center font-black text-3xl border-4 border-white shadow-md relative group select-none">
            {getInitials(nama)}
          </div>

          <div>
            <h2 className="text-sm font-black text-slate-800 uppercase leading-snug">{nama}</h2>
            <p className="text-[11px] text-slate-450 font-mono font-bold mt-0.5">{nim || "NIM Tidak Tercatat"}</p>
          </div>
          ...
          <div className="flex flex-col gap-2 w-full pt-4 border-t border-dashed">
            <div className="flex justify-between text-[11px] font-semibold text-slate-600">
              ...
              <span className="bg-amber-100 text-amber-800 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-wider border border-amber-250">
                {user.role}
              </span>
            </div>
            <div className="flex justify-between text-[11px] font-semibold text-slate-600">
              <span className="text-slate-400 uppercase font-mono">Gabung:</span>
              <span className="text-slate-700 font-mono">{user.tanggalBergabung || "31 Mei 2026"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-2 bg-white border border-slate-200/60 rounded-3xl p-6 shadow-sm">
          <h3 className="text-xs font-mono font-black uppercase text-[#114E8D] border-b pb-2 mb-4 tracking-wider">
             FORMULIR PERUBAHAN INFORMASI PROFIL
          </h3>
            ...
            <div className="space-y-1 pt-1">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  disabled
                  value={user.email}
                  className="w-full text-slate-400 font-mono font-medium pl-10 pr-4 py-3 bg-slate-100 border border-slate-200 rounded-xl outline-none text-xs h-11 select-all cursor-not-allowed"
                />
              </div>
              <p className="text-[9.5px] text-slate-400 font-semibold leading-snug">
                * Email kampus terdaftar bersifat permanen dan tidak dapat diedit karena digunakan untuk otorisasi verifikasi sertifikasi.
              </p>
            </div>
